...
Lplot<-ggplot(Ltemp,aes(r,obs))+geom_line(colour=c("#000000"))+
  ...
  theme(axis.title.y = element_text(face = "italic"),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
...
newtree<-function(year1,year2,species){
  ...
  if(nrow(newdt)>=15){
    ...
    mtext(paste("n=",nrow(newdt),sep = ""),line = -2,cex = 2,adj = 0.01)
...
newyear1<-matrix(c("91","97","05"))
newyear2<-matrix(c("97","05","13"))
...
#新增所有
newall<-sqldf("select * from db where dbh91=0 and dbh97>0")
if(nrow(newall)>=15){
  mypattern<-ppp(newall$x4,newall$y4,c(0,300),c(0,100))
  L<-envelope(mypattern, Lest, nsim = 99,correction="best")
  png(filename = "newall1997.png",width = 800, height = 645, units = "px")
  par(mar=c(5,6,5,3),cex.lab=2,cex.axis=2,cex.main=2)
  plot(L,.-r~r,ylab = expression(L(r)),xlab = "d(m)",main ="",legend=FALSE)
...
#新增與存活
dt1<-sqldf("select * from db where dbh91=0 and dbh97>0")
dt2<-sqldf("select * from db where dbh91>0 and dbh97>=8")
bidt<-rbind(dt1,dt2)
bidt$status91<-ifelse(bidt$dbh91==0,"new","survive")
m<-as.factor(bidt$status91)
mypattern<-ppp(bidt$x4,bidt$y4,c(0,300),c(0,100),marks = m)
L<-envelope(mypattern,Lcross,nsim = 99,i="new",j="survive")
png(filename = "newsurvive1997.png",width = 800,height = 645,units = "px")
par(mar=c(5,6,5,3),cex.lab=2,cex.axis=2,cex.main=2)
plot(L,.-r~r,ylab = expression(L(r)),xlab = "d(m)",main = "",legend=FALSE)
dev.off()
...
#新增與死亡
dt1<-sqldf("select * from db where dbh91=0 and dbh97>0")
dt2<-sqldf("select * from db where dbh91>8 and dbh97=0")
bidt<-rbind(dt1,dt2)
bidt$status91<-ifelse(bidt$dbh91==0,"new","dead")
m<-as.factor(bidt$status91)
mypattern<-ppp(bidt$x4,bidt$y4,c(0,300),c(0,100),marks = m)
L<-envelope(mypattern,Lcross,nsim = 99,i="new",j="dead")
png(filename = "newdead1997.png",width = 800,height = 645,units = "px")
par(mar=c(5,6,5,3),cex.lab=2,cex.axis=2,cex.main=2)
plot(L,.-r~r,ylab = expression(L(r)),xlab = "d(m)",main = "",legend=FALSE)
dev.off()
  ...
  if(datatype1=="new"){
    if(datatype2=="dead"){
      ...
      if(nrow(spdt1)>=15 & nrow(spdt2)>=15){
        ...
        mypattern<-ppp(bidt$x4,bidt$y4,c(0,300),c(0,100),marks = m)
        L<-envelope(mypattern,Lcross,nsim = 99,i="new",j="survive")
        png(filename = paste(spname,year3,"newsurvive_",dbhbig,".png",sep = ""),width = 800, height = 645, units = "px")
        par(mar=c(5,6,5,3),cex.lab=2,cex.axis=2,cex.main=2)
        plot(L,.-r~r,ylab = expression(L(r)),xlab = "d(m)",main = paste(year3,spname,"新增與存活大樹(DBH≧",dbhbig," cm)",sep=""),legend=FALSE)
        dev.off()
      }else if(nrow(spdt1)>=15 & nrow(spdt2)<15){
        dt2<-sqldf(paste("select * from db where dbh",year1,">0 and dbh",year2,">0",sep = ""))
        spdt2<-filter(dt2,sp==spname)
        bidt<-rbind(spdt1,spdt2)
        if(year2=="05"){
          year3<-"2005"
        ...
        mypattern<-ppp(bidt$x4,bidt$y4,c(0,300),c(0,100),marks = m)
        L<-envelope(mypattern,Lcross,nsim = 99,i="new",j="survive")
        png(filename = paste(spname,year3,"newsurvive.png",sep = ""),width = 800, height = 645, units = "px")
        par(mar=c(5,6,5,3),cex.lab=2,cex.axis=2,cex.main=2)
        plot(L,.-r~r,ylab = expression(L(r)),xlab = "d(m)",main = paste(year3,spname,"新增與存活",sep=""),legend=FALSE)
        dev.off()
