...
#ggplot
dt<-dbGetQuery(con,"select * from ljc2018")
dt$sp %<>% iconv(.,"UTF-8","CP950")
...
Lplot<-ggplot(Ltemp,aes(r,obs))+geom_line(colour=c("#000000"))+
  ...
  geom_hline(yintercept=0, linetype = "dashed", colour=c("#ff0000"))+
...
deadtree<-function(year1,year2,dbh){
  deaddt<-dbGetQuery(con,paste("select * from ljc2018 where dbh",year1,">=",dbh," and dbh",year2,"=0",sep = ""))
...
bivar<-function(datatype1,datatype2,year1,year2,spname,dbhbig){
  if(datatype1=="new"){
    if(datatype2=="dead"){
      year3<-""
      dt1<-dbGetQuery(con,paste("select * from ljc2018 where dbh",year1,"=0 and dbh",year2,">0",sep = ""))
      dt1$sp<-iconv(dt1$sp,"UTF-8","CP950")
      spdt<-filter(dt1,sp==spname)
      dt2<-dbGetQuery(con,paste("select * from ljc2018 where dbh",year1,">=",dbhbig," and dbh",year2,"=0",sep = ""))
      ...
      dt2<-dbGetQuery(con,paste("select * from ljc2018 where dbh",year1,">0 and dbh",year2,">",dbhbig,sep = ""))
      dt2$sp<-iconv(dt2$sp,"UTF-8","CP950")
